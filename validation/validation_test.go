...
func TestArray(t *testing.T) {
	...
	type Conn struct {
		Users []User `valid:"Required"`
	}

	type Context struct {
		Size  int  `valid:"Required"`
		Conns Conn `valid:"Required"`
	}

	valid := Validation{}
	c := &Context{Conns: Conn{Users: []User{{Name: ""}, {Name: "222"}}}, Size: 111}
	_, err := valid.Valid(c)
	if err == nil {
		t.Fatalf("expect Email error")
	}
}
