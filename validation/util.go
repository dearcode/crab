...
func getValidFuncs(f reflect.StructField) (vfs []ValidFunc, err error) {
	...
	aliase := f.Tag.Get(AliaseTag)
	if aliase == "" {
		aliase = f.Name
	}

	if vfs, tag, err = getRegFuncs(tag, aliase, f.Name); err != nil {
		return
	}
	...
	for _, vfunc := range fs {
		...
		vf, err = parseFunc(vfunc, aliase, f.Name)
		if err != nil {
			return
		}
		vfs = append(vfs, vf)
...
func getRegFuncs(tag, aliase, key string) (vfs []ValidFunc, str string, err error) {
	...
	vfs = []ValidFunc{{"Match", aliase, []interface{}{reg, key + ".Match"}}}
	str = strings.TrimSpace(tag[:index]) + strings.TrimSpace(tag[end+len("/)"):])
	return
}

func parseFunc(vfunc, aliase, key string) (v ValidFunc, err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("%v", r)
		}
	...
	if start == -1 {
		...
		v = ValidFunc{vfunc, aliase, []interface{}{key + "." + vfunc}}
		return
	}
